...
import Aside from "../../Components/PC/AsideGenre";
...
export default function ListMoviesGenre() {

    let { genre } = useParams()
    let { genreWrite } = useParams()
    ...
    function handlePageClick(event: Event) {
      setListMovies([])
      getMoviesByGenre(genre!, event.selected + 1)
    ...
    async function getMoviesByGenre(genre: string, page: number) {
      try {
        let response = {}  
        response = await axios.get(`${tmdbURL}discover/movie?language=pt-BR&page=${page}&sort_by=popularity&with_genres=${genre}`, {
          headers: {
        ...
        <div>
            ...
            <div>
              <div>
                ...
                <h1 className="ml-8 text-white font-bold text-3xl"> {genreWrite} </h1>
              </div>
                <div className="flex">
